...
parser.add_argument('--model_path', type=str, default='model/saved_models/mnist_classifier.pt',
...
parser.add_argument('--micro_batches', type=int, default=10, help='Number of micro-batches for training benchmark')
...
def measure_inference_time(model, loader, num_runs=100):
    ...
    print(f"Average inference time for single sample: {single_sample_time * 1000:.2f} ms")
...
def measure_training_speed(model, loader, num_batches=10):
    ...
    print(f"Average training time per batch: {training_time * 1000 / num_batches:.2f} ms")
...
def analyze_errors(model, loader, num_samples=20):
    ...
    for i, example in enumerate(misclassified_examples[:20]):
        ...
        axes[i].set_title(f"True: {example['true']} Pred: {example['pred']}\n"
