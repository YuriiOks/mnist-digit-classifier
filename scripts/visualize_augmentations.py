...
try:
    ...
    print(f"CRITICAL ERROR: Failed to import augmentation functions: {e}", file=sys.stderr)
    ...
    print("  - 'utils' contains '__init__.py' and 'augmentation.py'.", file=sys.stderr)
    print("  - This script is run from the project root (e.g., using 'python -m scripts.visualize_augmentations').", file=sys.stderr)
    sys.exit(1)
...
def get_transform_pipeline(transform_type: str) -> Tuple[Any, str]:
    ...
    if transform_type == 'train':
        ...
        return get_canvas_simulation_transforms(), "Canvas Simulation Augmentation"
...
def visualize_augmentations(transform_type: str, num_samples: int, seed: int):
    ...
    try:
        ...
        if len(raw_train_dataset) == 0:
             print("Warning: MNIST dataset appears empty after loading.", file=sys.stderr)
             return # Cannot proceed if dataset is empty
    except Exception as e:
        print(f"Error loading MNIST dataset from {MNIST_DATA_PATH}: {e}", file=sys.stderr)
    ...
    if actual_num_samples < num_samples:
        print(f"Warning: Requested {num_samples} samples, but dataset only has {len(raw_train_dataset)}. Visualizing {actual_num_samples}.", file=sys.stderr)
    if actual_num_samples == 0:
        print("Error: No samples available in the dataset to visualize.", file=sys.stderr)
        return
    ...
    # Create plot
    fig, axes = plt.subplots(actual_num_samples, 2, figsize=(6, 2.5 * actual_num_samples))
