...
def get_probabilities_labels(model: nn.Module, data_loader: DataLoader,
                             device: torch.device) -> tuple | None:
    ...
    with torch.no_grad():
        for images, labels in tqdm(data_loader, desc="Calibration Data"):
            try:
                ...
                logger.error(f"Error processing batch in "
...
def plot_reliability_diagram(confidences: np.ndarray,
    ...
    for i in range(num_bins):
        ...
        if bin_counts[i] > 0:
            bin_accuracies[i] = np.mean(predictions[in_bin] ==
                                        true_labels[in_bin])
    ...
    ax1.bar(bin_lowers[valid_bins], bin_accuracies[valid_bins],
            width=(1.0/num_bins)*0.9, align='edge', edgecolor='black',
    ...
    for i, idx in enumerate(np.where(valid_bins)[0]):
        ax1.plot([bar_centers[idx], bar_centers[idx]],
...
class _ECELoss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        for i in range(self.n_bins):
            in_bin = confidences.gt(self.bin_lowers[i]) \
                     & confidences.le(self.bin_uppers[i])
...
class ModelWithTemperature(nn.Module):
    ...
    def set_temperature(self, val_loader: DataLoader,
        ...
        try:
            ...
            all_labels = torch.cat(labels_list).to(device)
             ...
             logger.error(f"Failed to gather logits/labels for temp scaling:"
        ...
        def eval_nll_closure():
            ...
            loss.backward()
            logger.debug(f"  Temp: {self.temperature.item():.4f}, "
                         f"NLL: {loss.item():.4f}")
            return loss
        ...
        try:
             with torch.no_grad():
                  ...
                  ece_after = ece_criterion(scaled_logits_final,
                                            all_labels).item()
             logger.info(f"📊 ECE After Temp Scaling : {ece_after:.4f} "
                         f"(Optimal T={optimal_temp:.4f})")
             if ece_after >= ece_before - 1e-4: # Check if ECE meaningfully improved
                  logger.warning("⚠️ Temperature scaling did not significantly "
