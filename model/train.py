...
def get_data_loaders(
    ...
    try:
        ...
        val_dataset_view = datasets.MNIST(
            root=data_dir,
            train=True,
        ...
        logger.info(
            f"📊 Split training data: {len(train_subset)} train, "
...
def train_model(
    ...
    if best_epoch != -1:
        logger.info(f"🏆 Best model from Epoch {best_epoch}")
    else:
        logger.warning("⚠️ No best model saved.")
...
def optimize_temperature(
    model_path: str, device: torch.device, batch_size: int
) -> float:
    ...
    try:
        ...
        logger.info(
            f"💾 Optimal temperature {optimal_temp:.4f} saved to "
            f"{OPTIMAL_TEMP_FILE}"
        )
...
def final_evaluation(
    ...
    try:
        ...
        logger.error(f"⚠️ Failed save debug imgs: {e_save}")
    ...
    try:
        ...
        logger.error(f"🔥 Failed report gen: {e}")
    ...
    try:
        ...
        if confidences_np is not None:
            ...
            logger.info(
                f"📉 Reliability diagram saved: {reliability_plot_path}"
                f" | ECE={ece:.4f}"
            )
        else:
            logger.warning("⚠️ Skipping reliability plot: no data.")
    except Exception as e:
        logger.error(f"🔥 Failed reliability gen: {e}", exc_info=True)
