    ...
    for size in sizes:
        ...
        if use_mps:
            try:
                ...
                if not np.isnan(speedup): speedups.append(speedup)
...
def benchmark_mnist_training(num_iters=1, batch_sizes=[16, 32, 64, 128, 256], num_images=2000, use_mps=True):
    """Benchmarks MNIST training performance across different batch sizes."""
    print_header(f"MNIST Training Benchmark (1 Epoch on {num_images} images, Avg. {num_iters} iteration(s))")
    ...
    try:
        ...
        if actual_num_images < num_images:
            print(f"Warning: Requested {num_images} images, but MNIST train set only has {len(full_train_dataset)}. Using {actual_num_images}.")
    ...
    def run_epoch(model, optimizer, device, current_batch_size, dataset):
        ...
        train_loader = DataLoader(dataset, batch_size=current_batch_size, shuffle=True, num_workers=num_workers, pin_memory=(device.type != 'mps'))
        ...
        print(f"  Finished in {duration:.4f}s ({batch_count} batches, Avg Loss: {avg_loss:.4f})")
        return duration
    ...
    for batch_size in batch_sizes:
        ...
        print("  CPU Warm-up...")
        _ = run_epoch(model_cpu, optimizer_cpu, cpu_device, batch_size, train_dataset_subset)
        print("  Benchmarking CPU...")
        cpu_times = [run_epoch(model_cpu, optimizer_cpu, cpu_device, batch_size, train_dataset_subset) for _ in range(num_iters)]
        cpu_time = sum(cpu_times) / num_iters
        cpu_imgs_per_sec = actual_num_images / cpu_time if cpu_time > 0 else float('inf')
        ...
        if use_mps:
            try:
                ...
                print("  MPS Warm-up...")
                _ = run_epoch(model_mps, optimizer_mps, mps_device, batch_size, train_dataset_subset)
                _ = run_epoch(model_mps, optimizer_mps, mps_device, batch_size, train_dataset_subset)
                print("  Benchmarking MPS...")
                mps_times = [run_epoch(model_mps, optimizer_mps, mps_device, batch_size, train_dataset_subset) for _ in range(num_iters)]
                mps_time = sum(mps_times) / num_iters
                mps_imgs_per_sec = actual_num_images / mps_time if mps_time > 0 else float('inf')
                speedup = cpu_time / mps_time if mps_time > 0 and cpu_time > 0 else float('inf')
                print(f"  MPS Avg Time: {mps_time:.4f}s ({mps_imgs_per_sec:.1f} Img/s, Speedup: {speedup:.2f}x)")
        ...
        training_results.append([
            batch_size,
            f"{cpu_time:.4f}s", f"{cpu_imgs_per_sec:.1f}",
            f"{mps_time:.4f}s" if not np.isnan(mps_time) else "N/A",
            f"{mps_imgs_per_sec:.1f}" if not np.isnan(mps_imgs_per_sec) else "N/A",
            f"{speedup:.2f}x" if not np.isnan(speedup) else "N/A"
        ])

    # --- Print Summary Table ---
    print("\n--- Training Benchmark Summary ---")
    print(tabulate(training_results, headers=["Batch Size", "CPU Time", "CPU Img/s", "MPS Time", "MPS Img/s", "Speedup"], tablefmt="fancy_grid"))
    ...
    return best_batch_size_for_mps if best_mps_speedup > 1 else None # Return best BS only if MPS was faster
...
def visualize_speedups(matrix_results, inference_results, training_results=None):
    ...
    try:
        num_plots = 0
        if matrix_results: num_plots += 1
        if inference_results: num_plots += 1
        ...
        if matrix_results:
            ...
            speedups = [float(r[3].replace('x','')) if r[3] != "N/A" else 0 for r in matrix_results]
            ...
            for i, v in enumerate(speedups):
                ax.text(i, v + 0.05 * max(speedups, default=1), f"{v:.2f}x", ha='center', va='bottom', fontsize=9)
            plot_idx += 1
        ...
        if inference_results:
            ...
            speedups = [float(r[3].replace('x','')) if r[3] != "N/A" else 0 for r in inference_results]
...
def main():
    ...
    print(tabulate([
        ...
        ["📊 Matplotlib", getattr(plt.matplotlib, '__version__', 'N/A') if hasattr(plt, 'matplotlib') else 'N/A'] # Safer access
    ], tablefmt="fancy_grid"))
    ...
    if mps_available:
        ...
        if best_training_batch_size:
             print(f"🚀 MPS Training was faster than CPU. Best performance at batch size: {best_training_batch_size}.")
             ...
             print("👉 Consider using CPU for training unless further optimization is done (e.g., different model/optimizer).")
