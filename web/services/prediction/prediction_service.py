...
try:
    ...
    logging.warning("psycopg2 not available. Database functionality will be disabled.")
...
class PredictionService:
    ...
    def __init__(self):
        ...
        if PSYCOPG2_AVAILABLE and self.log_to_db:
            ...
            self.logger.warning("Database logging is disabled or psycopg2 is not available")
    ...
    def _check_and_setup_database(self) -> None:
        ...
        if not PSYCOPG2_AVAILABLE:
            self.logger.warning("psycopg2 not available. Cannot set up database.")
            return
            
        conn = None
        try:
            # Log connection attempt
            self.logger.info(f"Attempting to connect to database at {self.db_config['host']}:{self.db_config['port']}")
            ...
            self.logger.info("Database connection successful and tables verified")
    ...
    def get_db_connection(self) -> Optional[Any]:
        ...
        """
        if not PSYCOPG2_AVAILABLE:
            self.logger.warning("psycopg2 not available. Cannot connect to database.")
            return None
    ...
    def log_prediction(
        ...
        try:
            ...
            cursor.execute(query, (prediction, confidence, true_label, input_type))
    ...
    def get_prediction_history(
        ...
        """
        if not PSYCOPG2_AVAILABLE:
            self.logger.warning("psycopg2 not available. Cannot retrieve history.")
            return [], 0
        ...
        try:
            ...
            valid_sort_columns = ["timestamp", "prediction", "confidence", "id"]
    ...
    def update_true_label(self, prediction_id: int, true_label: int) -> bool:
        ...
        """
        if not PSYCOPG2_AVAILABLE:
            self.logger.warning("psycopg2 not available. Cannot update true label.")
            return False
    ...
    def delete_prediction(self, prediction_id: int) -> bool:
        ...
        """
        if not PSYCOPG2_AVAILABLE:
            self.logger.warning("psycopg2 not available. Cannot delete prediction.")
            return False
        ...
        try:
            ...
            success = rows_affected > 0
            if success:
                self.logger.info(f"Deleted prediction with ID {prediction_id}")
            else:
                self.logger.warning(f"No prediction found with ID {prediction_id}")
    ...
    def clear_history(self) -> bool:
        ...
        """
        if not PSYCOPG2_AVAILABLE:
            self.logger.warning("psycopg2 not available. Cannot clear history.")
            return False
        ...
        try:
            ...
            self.logger.info(f"Cleared prediction history. {rows_affected} records deleted.")
            return True
