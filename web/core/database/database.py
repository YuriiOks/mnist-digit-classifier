...
class Database:
    ...
    def insert_prediction(self, prediction: int, confidence: float, true_label: Optional[int] = None,
                        input_type: Optional[str] = None, image_data: Optional[str] = None) -> Optional[int]:
        ...
        if image_data and len(image_data) > 100000:  # 100KB limit
            image_data = None
            self.logger.warning("Image data too large for database storage. Skipping image.")
        
        result = self.execute_query(query, (prediction, confidence, true_label, input_type, image_data), fetch=True)
    ...
    def get_predictions(self, limit: int = 20, offset: int = 0, 
                      digit_filter: Optional[int] = None,
                      min_confidence: float = 0.0) -> Tuple[List[Dict[str, Any]], int]:
