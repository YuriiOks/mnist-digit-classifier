...
class SettingsState:
    ...
    def initialize(cls) -> None:
        ...
        if not SessionState.has_key(cls.SETTINGS_KEY):
            ...
            default_settings = {
                ...
                # Prediction settings
                "prediction": {
                    "auto_predict": True,     # auto-predict on drawing completion
    ...
    def set_setting(cls, category: str, key: str, value: Any) -> None:
        ...
        logger.debug(f"Updated setting {category}.{key} to {value}")
    ...
    def update_category(cls, category: str, values: Dict[str, Any]) -> None:
        ...
        SessionState.set(cls.SETTINGS_KEY, settings)
        logger.debug(f"Updated {category} settings with {values}")
    ...
    def reset_to_defaults(cls, category: Optional[str] = None) -> None:
        ...
        if category:
            ...
            if category in settings:
                ...
                SessionState.set(cls.SETTINGS_KEY, settings)
                logger.debug(f"Reset {category} settings to defaults")
        else:
            # Reset all settings
